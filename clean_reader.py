...
def get_equal_for_each_cat(data_set,max_length):
    ...
    print("minimum length of each cat: ",min([len(data_set_1),len(data_set_3),len(data_set_5)]))
    #is needed this is set with a number above 0
    if max_length > 0:
        data_set_3 = data_set_3.sample(n=max_length)
        data_set_1 = data_set_1.sample(n=max_length)
        data_set_5 = data_set_5.sample(n=max_length)

    data_frames = [data_set_3,data_set_1,data_set_5]
    dataset = pd.concat(data_frames)
    dataset = dataset.sample(frac=1).reset_index(drop=True)

    print("length of dataset equal from each",len(dataset))
    print(dataset[:10])
...
def clean_text_language(dataset):
    ...
    #unnecessary function due to tokenizer doing th e same
...
#####################################################
def load_data(path = 'yelp_review.csv'):
    print("loading data...")
...
# Create the model. #
#####################
def create_model(embedding_layer, dropout_size, input_dim, model_type,use_cnn):
    model = Sequential()
    #Create LSTM model
...
def kfold_cross_run(dataset_size,encode_tfidf_bool,
            model_type,use_cnn,feature_rep_path,
    ...
    for train,test in kfold.split(x,Y_index):
        ...
        model = create_model(embedding_layer, dropout_size, MAX_SEQUENCE_LENGTH, model_type,use_cnn)
        model.compile(loss = 'categorical_crossentropy',
                      optimizer = 'adam',
                      metrics = ['accuracy'])
        ...
        list_history.append(history.history)
    ...
    model = load_model('./modelCheckPoints/modelCheckpoint_{!s}_{!s}'.format(model_type,feature_rep_type))
    preds = model.predict(x_test)
    y_true = [np.argmax(i) for i in y_test]
    preds = [np.argmax(i) for i in preds]
    #cvscores.append(scores[1] * 100)
    print(confusion_matrix(y_true,preds))
    print(accuracy_score(y_true,preds,normalize = True)*100,"%")
    print("%.2f%% (+/- %.2f%%)" % (np.mean(cvscores), np.std(cvscores)))
    plot_scores(list_history)
#####################################
...
#            feature_rep_type = 'ft-fb', dropout_size = 0.4)
    ...
    kfold_cross_run(dataset_size = 33000, encode_tfidf_bool = False,
            model_type = 'mlp',use_cnn = False feature_rep_path = 'None',
            feature_rep_type = 'ft-fb', dropout_size = 0.0,folds = 10)
    #data = load_data()
    #text_setup_for_feature_representation(data,1)
